...
test.describe('스냅문서 E2E', () => {
  test.beforeEach(async ({ page }) => {
    // 한국어 강제
    await page.goto('/');
    await page.evaluate(() => localStorage.setItem('snapdoc-locale', 'ko'));
    await page.reload();
  });

  test.beforeAll(() => {
    createTestXlsx([['이름', '생년월일', '연락처', '주소', '특이사항']], 'template_test.xlsx');
    createTestXlsx([['성명', '생년월일', '전화번호', '거주지', '비고'], ['홍길동', '1960-05-20', '010-1234-5678', '서울시 강남구', '당뇨']], 'data_test.xlsx');
  });

  test('페이지 로드: 한국어 텍스트 확인', async ({ page }) => {
    await expect(page.locator('.logo')).toHaveText('스냅문서');
    await expect(page.locator('.beta-badge')).toHaveText('테스트 사이트');
  });

  test('템플릿: 엑셀 업로드 → 컬럼 분석', async ({ page }) => {
  ...
  test('정보문서: 파일 업로드 → 미리보기', async ({ page }) => {
    const fileInputs = page.locator('input[type="file"]');
    await fileInputs.last().setInputFiles('e2e/data_test.xlsx');
    await expect(page.locator('.step-title').filter({ hasText: '미리보기' })).toBeVisible();
    await expect(page.locator('textarea')).toHaveValue(/홍길동/);
  });

  test('정보문서: 텍스트 입력 → 변환 버튼 확인', async ({ page }) => {
    await page.locator('textarea').fill('이름: 테스트');
